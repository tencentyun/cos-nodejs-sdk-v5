...
## [v3.0.0](https://github.com/tencentyun/cos-nodejs-sdk-v5/compare/v2.15.0...v3.0.0) - 2026-07-10

### **BREAKING CHANGES**

- **Node.js 最低版本要求提升至 `>= 9.0.0`**（不再支持 Node 6 / 7 / 8）
- 移除依赖库 `request`，改为自维护的 `cos-request`
- 移除依赖库 `fast-xml-parser`，改为自维护的 `cos-fast-xml-parser`
- 移除依赖库 `conf`，改为内置的零依赖轻量实现 `conf-lite`

### Features

- 支持严格签名：`content-type`、`cache-control`、`content-length` 加入签名计算
- 支持 COS / CI 备用域名自动切换（COS 主域名切换至 `tencentcos.cn`，CI 主域名切换至 `tencentci.cn`），并按响应状态码精细化重试策略
- `CompleteMultipartUpload` 及 copy 类接口（putObjectCopy / uploadPartCopy）返回 2xx 但响应体含 Error 时，正确返回错误并支持重试
- `downloadFile` 支持透传自定义 Headers

### Fixed

- 优化 XML 解析：关闭默认 trim、调整实体扩展上限，修复文件列表解析报错

## [v2.16.0-beta.0](https://github.com/tencentyun/cos-nodejs-sdk-v5/compare/v2.15.0...v2.16.0-beta.0) - 2025-04-24
